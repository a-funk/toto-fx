...
export function createDotgrid(userConfig) {
  ...
  var publicApi = {
    ...
    // Backward-compat convenience methods — delegate to runEffect
    ripple:  function (cx, cy, opts) { runEffect('ripple',  { cx: cx, cy: cy, opts: opts }); },
    crater:  function (cx, cy, radius, depth, opts) { runEffect('crater',  { cx: cx, cy: cy, opts: Object.assign({ radius: radius, depth: depth }, opts || {}) }); },
    nuclear: function (cx, cy, opts) { runEffect('nuclear', { cx: cx, cy: cy, opts: opts }); },
    scorch:  function (x1, y1, x2, y2, widthOrOpts) { var o = typeof widthOrOpts === 'number' ? { width: widthOrOpts } : widthOrOpts || {}; runEffect('scorch', { x1: x1, y1: y1, x2: x2, y2: y2, opts: o }); },
    vortex:  function (cx, cy, opts) { runEffect('vortex',  { cx: cx, cy: cy, opts: opts }); },
    heart:   function (cx, cy, opts) { runEffect('heart',   { cx: cx, cy: cy, opts: opts }); },
  };
